...
## 质量门禁（改完必跑）
```
npm run typecheck && npm run build      # 类型 + 静态预渲染（现 212 页：7 计算器 base + 44 预设 × 4 语言）
...
当前 7 个计算器：schengen / fire / tdee / compound / mortgage / body-fat-bmi / auto-loan（finance 类 4 个）。
